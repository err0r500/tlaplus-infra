...
vars == <<confOK, reqCounter, lastVOK, toApply, cluster, requests, workers>>
...
Submit(r) == \* update request received from the user 
    ...
    /\ UNCHANGED <<confOK, lastVOK, toApply, cluster, workers>>
...
Initialcheck(r) == \* request validation (auth, quotas...)
    ...
    /\ UNCHANGED <<confOK, reqCounter, lastVOK, toApply, cluster, workers>>
...
PushToPending(r) == \* the request is pushed to queue
    ...
    /\ IF toApply < requests[r].v
        THEN /\ toApply' = requests[r].v
             /\ UNCHANGED <<confOK, reqCounter, lastVOK, cluster, requests, workers>>
